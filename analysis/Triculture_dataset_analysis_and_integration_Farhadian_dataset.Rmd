...
Only 5 genes overlap when you overlap the genes up- or downregulated between
infected and uninfected cells in treatment versus no treatment.
```{r}
...
##### Volcano Plot of Infection (+treatment) signature
...
inf_uninf_art_marker %>% 
  rownames_to_column("gene") %>%
  filter(gene %in% c(up_inf$gene, down_inf$gene)) %>% 
  # add a column containing information in which condition the corresponding TF 
  # is upregulated
  mutate(condition = case_when(gene %in% up_inf$gene ~ "upregulated",
                               gene %in% down_inf$gene ~ "downregulated",
                               TRUE ~ "NA")) %>%
...
inf_up <- intersect((treatment_marker_inf %>% 
             rownames_to_column("gene") %>% 
...
To identify features that are differentially expressed between cells traeted
with EFZ vs cells not treated with EFZ we first use the Uninf group as baseline and 
compare it to the Uninf+EFZ group, then we compare Inf group as baseline
to Inf+EFZ group. Finding an overlap between the two resulting signatures should
return the genes which define either condition group. 


```{r, results = "asis"}
# find markers between Inf+EFZ and Uninf
treatment_marker_uninf <- FindMarkers(microglia, ident.1 = "Uninf+EFZ", 
                                ident.2 = "Uninf")
...
treatment_marker_uninf %>% 
  ...
  geom_hline(yintercept = -log10(1e-10), col = "red") +
  geom_text(nudge_x = .05, nudge_y = 2) +
  labs(title = "Genes which are up- or downregulated in EFZ treatment vs. no treatment, Uninf cells")
```
...
a different approach and just use the top 20 up- and downregulated genes in 
Inf+EFZ vs Uninf+EFZ. Using these genes as an "infection signature" I will 
check if some of the Myeloid2 cells from the CSF samples show a similar signature.


```{r}
# select only genes which are also found in the sc-RNAseq dataset
up_inf_filt <- up_inf$gene %in% rownames(hivmy2@assays$RNA@counts)
down_inf_filt <- down_inf$gene %in% rownames(hivmy2@assays$RNA@counts)
```
...
Below, for simplicity, one plot is shown for upregulated genes and one plot for
downregulated genes.
...
inf_sign <- AddModuleScore(hivmy2,
                           features = up_inf_art$gene)

p1 <- FeaturePlot(inf_sign, features = "Cluster1", pt.size = .7) +#, split.by = "ht_cluster") +
...
```{r, fig.width=15}
p1 <- FeaturePlot(subset(inf_sign, ht_cluster == "c1"),
                  features = "Cluster1", pt.size = 1) +#, split.by = "ht_cluster") +
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu"))) +
  labs(title = "Myeloid2 Cluster c1")

p2 <- FeaturePlot(subset(inf_sign, ht_cluster == "c2"),
                  features = "Cluster1", pt.size = 1) +#, split.by = "ht_cluster") +
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu"))) +
...
percentage_inf <- dim((inf_sign@meta.data["Cluster1"] %>% 
                         filter(Cluster1 > 0)))[1] / 
  dim(inf_sign@meta.data["Cluster1"])[1]

# show the positive scores.
inf_sign@meta.data["Cluster1"] %>% 
  filter(Cluster1 > 0) %>% 
