...
## De-nove clustering of the myeloid-lineage cluster of the integrated dataset
...
Only 3% of genes which are upregulated and only 11% of genes which are 
...
heatmap below, you do not really see a difference between the groups. 
...
treat_cells <- rownames(up_treat@meta.data %>% filter(Cluster1 > 0))
```
...
high scores for the Inf+EFZ signature compared to cells without the signature.
Since no significant differences were found with the Kruskal-Wallis test, no 
further posthoc analysis was performed.
