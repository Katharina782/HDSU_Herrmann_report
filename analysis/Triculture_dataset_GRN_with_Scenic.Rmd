...
## Gene regulatory network (GRN) inference with SCENIC

To infer GRNs from iMg cells from the triculture dataset [@Ryan] (4333 cells in 
total) I again used SCENIC.

When referring to differentially bound TFs in the C20 cell line in the following
...
I first tried to run SCENIC with 3000 most highly variable genes (HVG), but then
I only got an overlap with differentially bound TFS of eight. When using 10,000
HVG SCENIC returns 232 TFS and their corresponding activity score for each cell.
25 of these TFs overlap with differentially bound TFs in the C20 cell line.
...
<details>
<summary>**preparing matrix for SCENIC**</summary>

First, I use SCTransform for normalization of the expression data, then
the top 10,000 HVGs are selected and the matrix transposed, because the pySCENIC
pipeline used requires an in input matrix which has genes as columns and cells 
as rows.
...
## Footprinting binding scores from C20 cell line
...
and twenty TFs are more bound in uninfected cells

The reason why we find more of the TFs more bound in uninfected cells 
...
#### Hierarchical clustering with different k {.tabset}
...
206 TFs have different acivity between the clusters. 
...
additionally following the pattern of the footprinting. For example, MEF2C is
more bound in the uninfected iMg than in infected iMg.
