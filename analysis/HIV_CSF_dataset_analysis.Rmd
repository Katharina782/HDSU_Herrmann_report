---
title: "Analysis of HIV-1 infected CSF & Blood samples"
...
First, the analysis done by Farhadian et al. [@Farhadian] was reproduced and then a more extended
analysis of the data set was performed. The microglia-like cell cluster identified
by Farhadian et al. [@Farhadian] was of particular interest.

The data set includes three CSF samples from HIV infected patients, two blood 
samples from the same HIV-1 infected patients as two of the CSF samples, as well
as, two CSF samples from uninfected
patients. In the following HIV was written for simplicity, but all data and 
analysis refers to HIV-1. [@Farhadian] first used two CSF and two bloods samples 
from the same two infected
patients for clustering and cell type annotation. After having identified a 
novel subcluster of myeloid cells, which are proposed to be microglia-like, the 
results are confirmed using the third CSF samples from an infected patient.
Additionally, the number of Myeloid2-specific transcripts are compared to 
the CSF samples from uninfected patients. It is shown that the percentage of Myeloid2
transcripts was higher in CSF from infected patients than in CSF from uninfected
patients and higher in CSF than in blood in general. In the following analysis
the existence of a microglia-like group of cells was confirmed, however, 
the percentage of Myeloid2 transcripts found by Farhadian et al. [@Farhadian] could not be 
recapitulated.

Blood and CSF were processed separately for sc-RNAseq using SeqWell. The seven
samples were sequenced separately, one SeqWell array per CSF or PBMC sample
[@Farhadian]. For more details see [@Farhadian].
...
<details> 
<summary>Dimensionality reduction & Clustering</summary>

For further analysis blood and CSF samples from two patients (HIV1_CSF/Bld, HIV2_CSF/Bld) were used as in the paper.
...
```{r}
hiv4 <- hiv4 %>%  NormalizeData(verbose = FALSE) %>% 
  ScaleData(verbose = FALSE) %>% FindVariableFeatures(verbose = FALSE)
...
* Dimensionality reduction
  ...
  + **PCA:** Compute PCA for highly variable genes:
  The first 10 dimensions seem to account for most of the variance (see elbow plot).
  The scatter plots below show that the blood samples and csf samples are 
  well separated by the first two PCs.
...
p3 <- cbind(meta, tsne_coord) %>% 
  ...
  guides(color = guide_legend(title = "Sample origin"))
...
# Cell types and sample origin

Details on the cell type annotation can be found in the appendix under 
*"Marker gene expression of predicted cell types after label transfer"*. The 
tSNE embedding with the annotated cell types resembles the results by
[@Farhadian]. There are two clusters, Myeloid2 and Myeloid5 which mainly contain
cells from CSF. These two clusters were termed microglia-like by Farhadian et al. [@Farhadian]. 
...
cells. This reflects the findings of [@Farhadian] very nicely. Myeloid4 seems to be a patient-specific cell type since it is only found in blood of patient one 
(HIV1_Bld). The mixed population is primarily composed of blood cells from 
patient two (HIV2_CSF). Interestingly, CD8+ T-cells were found with to a  higher 
extent in CSF samples than in blood samples.
...
a few Myeloid5 cells (thirteen) originate from blood samples. 
...
### Marker genes for Myeloid2

Which genes are upregulated in Myeloid-2 compared to the four other Myeloid subsets?
In the paper 60 genes were found to be upregulated compared to the other 4 
myeloid subsets (FDR < 0.01).

In order to confirm that these 60 genes are upregulated in Myeloid2 compared 
to the four other Myeloid subsets, we will first identify differentially
expressed genes between the groups.
`FindAllMarkers()` compares one cluster to all other clusters, whil
`FindMarkers()` will compare an indicated cluster with a second indicated cluster. 
Therefore, we will use `FindMarkers()`, because then the difference will not be diluted, as might be the case when comparing one cluster to all other clusters,
...
#### 60 Myeloid2 marker genes

The dotplot below shows the expression of the 60 genes identified in the paper as Myeloid2 markers. The genes show the highest expression in Myeloid2. Some genes like CD74 and genes of the HLA family are also highly expressed in Myeloid5 cells. However, there are also a few genes, like FAU, TPT1, TMSB10 and MIR4426 which do not show  specificity for the Myeloid2 cluster. It is unclear why [@Farhadian] chose these 
unspecificgenes as signature genes.
...
### Frequency of Myeloid2 transcripts 

Next, [@Farhadian] determined the frequency of Myeloid2 transcripts in Blood and 
CSF samples from infected patients and in uninfected CSF samples.
...
compared to all other samples which contain between 932 and 2520 cells (see 
table below). However, also the CSF sample from patient1 (HIV1_CSF) with 932
cells contains much fewer cells compared to all other samples. 

The results shown by Farhadian et al. [@Farhadian] are controversial, because when the authors compare
the percentage of Myeloid2 transcripts between the three conditions, 1770 cells 
are pooled for the uninfected condition, which is a lot more than we find
here. Yet, it is not mentioned that different thresholds were used for quality 
control of the uninfected samples. Therefore, I was not able to reproduce the 
findings in the paper. Below, I will describe the percentage of Myeloid2 transcripts
with the number of cells obtained when using the 
...
The table below shows that the mean number of Myeloid2 transcripts of both 
infected and uninfected CSF samples is slightly higher than the mean of blood 
samples. The mean is even higher in uninfected compared to infected CSF samples. However, the median is highest in blood samples, followed by infected CSF samples
and it is lowest in uninfected CSF samples. To see if there is a difference between the three conditions a Kruskal-Wallis-Test
...
for the uninfected condition. P-values are listed in the table below the boxplots.

Only the p-value for the comparison between infected CSF vs uninfected CSF is
the same as in the paper, where it is stated that the Myeloid2 transcripts were
...
by Farhadian et al. [@Farhadian]. Contradictory to what is claimed in the paper, it is not evident
that both infected and uninfected CSF samples have higher rates of Myeloid2 
transcripts, but rather the Blood samples seem to have the highest rate of Myeloid2 
transcripts according to the median (see boxplots below). 

In general, the comparison drawn is questionable, because what the boxplots below
...
both CSF samples. This might be a hint that in the CSF samples there is an 
...
Myeloid2 cells in a particular sample, because Myeloid2 transcripts are expressed 
to some extent in all cell types.
...
p2 <- percentage_my2_transcripts %>% ggplot() +
  ...
  xlab("Sample origin") +
  ylab("Frequency of myeloid2 transcripts per cell") +
  labs(title = "Remove outliers to zoom in") +
  ylim(-0.01, 0.05)
  NoLegend()
...
* 1 CSF sample from HIV infected patient
* 2 CSF samples from uninfected patients

The frequency of each cell type in the newly annotated samples can be seen in
the table below. Myeloid1, Myeloid3 and Myeloid4 are not or hardly present in the 
newly annotated CSF samples, because they were mainly found in blood samples.
However, there are 132 cells annotated as Myeloid2 and 43 cells annotated as
Myeloid5. The Label Transfer seems to work nicely, judging from the correct 
expression of common cell type markers for the individual cell types. Visualizations
of the marker gene expression can be found in the appendix under *"Cell type*
*annotation with canonical marker genes"*.
...
<details> 
<summary>Table with number of cells of each cell type</summary>

```{r}
as.data.frame(table(query$predicted.id)) %>% 
  column_to_rownames("Var1") %>% 
  rename(n = Freq) %>% 
  kable(caption = "Numbers of cells of each cell type in query samples")
...
The barplots below show how the cell types are distributed across the four 
samples before label transfer (left) and when considering the 3 additional
HIV+/- CSF samples after label transfer.
...
p2 <- hiv7@meta.data %>%
  ...
  ylab("Number of cells in each cluster") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.8, hjust=1)) +
  labs(title = "Cell types per sample before label transfer")
...
To summarize the results presented above, the analysis of the data set revealed
a microglia-like group of Myeloid cells as described by Farhadian et al. [@Farhadian]. In 
the appendix more details can be found on how we independently confirmed the 
microglia-like signature of these Myeloid2 cells using marker genes described 
in the literature. We found 
that the frequency of Myeloid2 transcripts was higher in infected blood and CSF
samples than in the CSF sample from uninfected patients and the median frequency
was higher in infected blood than in infected CSF. Conversely Farhadian et al. [@Farhadian] 
found that the frequency of Myeloid2 transcripts was higher in both infected
and uninfected CSF samples than in infected blood samples. Since details on 
how data was processed in the paper are not available it is difficult to say 
how the authors arrived at this conclusion. In addition to the standard analysis
steps performed in the paper we performed 
a label transfer of cell types to the three remaining samples (1 infected CSF, 
2 uninfected CSF). Using the gene expression data of the CSF samples we will
in the following do gene regulatory network (GRN) inference to gain additional insights.
...
In the following, details on marker gene expression and cell type annotation are
shown. Cell type annotation was first performed for four samples, two blood and 
two CSF samples, from infected patients. Later on these labels were transferred 
to the remaining three samples. Marker gene expression after label transfer is
...
two groups of cells. In this case one group of cells at a time was compared against
...
p1 <- VlnPlot(hiv4, features = c("MS4A1", "CD79A", "TCL1A"), pt.size = .01, 
              group.by = "cell_type")
p2 <- RidgePlot(hiv4, features =  c("MS4A1", "CD79A", "TCL1A"), 
                sort = "decreasing", group.by = "cell_type" )
...
NKG7 and GNLY are also expressed in clusters 1 and 2.
GZMB, a NK cell marker is also found in cluster 14.

```{r, fig.width=15, fig.height=12}
p1 <- VlnPlot(hiv4, features = c("NKG7", "GNLY", "GZMB"),
        pt.size = .01, ncol = 3, group.by = "cell_type")#, stack = TRUE)
p2 <- RidgePlot(hiv4, features = c("NKG7", "GNLY", "GZMB"), 
                sort = "decreasing", group.by = "cell_type")
...
* CD8A is expressed in decreasing order in 1, 5 and 4

**CD4+ T cells:**

* CD3E, a general T cell marker is expressed in cluster 0, 1, 3, 4, 5 and 6.
* IL7R in clusters 0-6 and to some extent in cluster 13
* TRAC in clusters 0-6, excluding 2
* CCR7 in 0, 3 and 7
* LDHB does not show a very specific expression
* LTB in 0-7 and in cluster 13
* CD2 in 0-6, 9 and 12
...
Myeloid1, 3, 4 are assigned arbitrarily, because they are not of particular interest. 


```{r, fig.width=15, fig.height=15}
p1 <- VlnPlot(hiv4, features = c("LYZ", "FCER1A", "CD14", "FCGR3A"), 
                                 group.by = "cell_type", pt.size = .01, ncol = 2)
p2 <- RidgePlot(hiv4, features = c("LYZ", "FCER1A"), group.by = "cell_type",
          sort = "decreasing", ncol = 2) # DC markers
p3 <- RidgePlot(hiv4, features = c("CD14", "FCGR3A"), group.by = "cell_type",
...
### Myeloid 5
 ...
 Cluster 12 are Myeloid5 cells, which were characterized by Farhadian et al. [@Farhadian] with 
 the markers FCER1A and CD1C  

```{r, fig.width=15, fig.height=12}
# Myeloid 5 in the paper
p1 <- VlnPlot(hiv4, features = c( "FCER1A", "CD1C"), pt.size = .01, ncol = 2,
              group.by = "cell_type")
p2 <- RidgePlot(hiv4, features = c("FCER1A", "CD1C"), sort = "decreasing", 
...
# create named vectors of cell types
...
## Independent annotation of Myeloid2 and Myeloid5 
...
Gene signature defined by Esaulova et al. [@Esaulova]
...
* 10 genes were used to discriminate microglia, four of which have not been 
inlcuded [@Farhadian]: **CLEC9A**, **TIMD4**, **TNFSF18**, **CXCL12**
* the genes **CLEC9A**, **TIMD4**, **TNFSF18**, **CXCL12** are not included in
the 60 genes used by Farhadian et al. [@Farhadian], but they do not show specific expression
for Myeloid2 cells
...
28 of the 60 genes used as microglia signature in the paper [@Farhadian]  are also 
found in the 881 microglia signature genes defined by Gosselin et al. [@Gosselin].
...
of TFs provided by Gosselin et al. [@Gosselin].
...
macrophages. The authors identified 14 genes that are conserved in microglia throughout development,
...
The marker genes mentioned by Keren_Shaul et al. [@Keren-Shaul] and [@Kraseman] were referred to 
by Farhadian et al. [@Farhadian] and used as marker genes for microglia associated with 
...
* The signature genes defined by Keren-Shaul et al. [@Keren-Shaul] are expressed to some level 
in all myeloid clusters
* The signature defined by Kraseman et al. [@Kraseman] is also mainly expressed in the 
Myeloid clusters
...
###  Conclusions on Myeloid2 cell type
...
referred to by Farhadian et al. [@Farhadian]. Nevertheless, there are also many genes which 
...
## Marker gene expression of predicted cell types after label transfer{.tabset}
...
DotPlot(object = query, features = c("MS4A1", # B cells
                                    "TRAC", "IL7R", 
                                    "CD8A", # CD8 T cells
                                    "NKG7", "GNLY", # NK cells
                                    "CD3E", # T cells
