...
>>>>>>> 813ec51c3741eb1435e1daafde4dd0294bedd5e3
...
GZMB is also found in cluster 14.
...
Tcells1/2/3 are assigned arbitrarily, because they are not used for further analysis.
...
* cluster 12
* was characterized by [@Farhadian] with the markers FCER1A, CD1C  
...
* AXL, APOE, TREM2 (these genes are enriched in neurodegenerative 
  disease-associated microglia [@Farhadian])
...
# tSNE visualizations of cell types, conditions, samples and sequencing quality
...
=======
  summarise(number_of_cells = n()) %>% kable
>>>>>>> 813ec51c3741eb1435e1daafde4dd0294bedd5e3
```
...
=======
  labs(title = "Removed outliers to see difference") +
  stat_pvalue_manual(percentage_my2_transcripts %>% 
    rstatix::pairwise_wilcox_test(percentage ~ condition) %>% 
      rstatix::add_xy_position()) +
  ylim(0, 0.07) +
...
#hiv4 <- readRDS("seurat_objects/Seurat_objects/non_integrated_HIV1_HIV2_4samples_seurat_object.rds")
```
...
Myeloid 1, 3, 4 are not/hardly present in the CSF samples which is expected, 
since they originated exclusively from the Blood samples in the reference dataset.
...
### Check the gene expression levels in the predicted cell types {.tabset}
...
In the following part we tried to confirm that the Myeloid2 (and maybe the 
...
do.call(grid.arrange, c(append(tsne_list, dotplot_list), ncol = 3, nrow = 2))
```
...
do.call(grid.arrange, c(cluster_plots, ncol = 4, nrow = 2))
