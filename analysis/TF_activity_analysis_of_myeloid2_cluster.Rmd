---
title: "TF-activity analysis of myeloid2 cells"
author: "kmikulik"
date: "7 12 2021"
...
# 1. Determine k for k-means hierarchical clustering {.tabset}

The plot of within-cluster som of squares shows that the sweet spot for the 
...
Since k = 4 and k = 5 does not yield an increased number of TFs which differntiate
the different clusters, and therefore do not add any insights we proceeded with
k = 3 in the following analysis.

The results of each k can be seen below. Click on the tabs to see heatmaps of 
the hierarchical clustering, as well as statistical test on different TF
acitivity between clusters.
...
ANOVA test is parametric and assumes normally distributed data, while
Kruskal-Wallis-Test compares the mean of one or more groups, but is 
non-parametric. As can be seen in the boxplots above the data is not normally 
distributed, therefore Kruskal-Wallis-Test is more appropriate in this case.
...
Performing Kruskal-Wallis-Test to find differences between the three clusters in patient1
yields 54 TFs of all 125 TFs which are differentially active between the three clusters
(p-value < 0.05). Of the 24 TFs with differential binding scores in the C20 cell 
line, sixteen have significantly different activity between the clusters.
...
Performing Kruskal-Wallis-Test to find differences between the four clusters in patient1
yields 51 TFs of all 125 TFs which are differentially active between the three clusters
(p-value < 0.05). Of the 24 TFs with differential binding scores in the C20 cell 
line, fourteen have significantly different activity between the clusters.
...
Performing Kruskal-Wallis-Test to find differences between the five clusters in patient1
yields 59 TFs of all 125 TFs which are differentially active between the three clusters
(p-value < 0.05). Of the 24 TFs with differential binding scores in the C20 cell 
line, fifteen have significantly different activity between the clusters.
...
## Which Clusters are sifnificantly different for different k? {.tabset}
...
# 3. Hierarchical clustering for all 3 patients separately {.tabset}
...
Since the independent hierarchical clustering for all three patients yields 
...
## 11 TFs show different activity in one of the three clusters {.tabset}
...
<button onclick="myFunction()">*Click here for details on how I computed the correlation*</button>
...
<button onclick="myFunction()">*Click here for some additional correlation plots which do not aid the interpretation much.*</button>
...
For these eleven TFs with different activity between the clusters the activity 
correlates between across all three patients. This means, that the three TFs
...
there might be a biological meaning behind the pattern of activity. 
...
<button onclick="myFunction()">*Click here for boxplots of housekeeping gene expression*</button>
...
<button onclick="myFunction()">*Click here for boxplots of microglia marker genes*</button>
...
<button onclick="myFunction()">*Click here for details on differential gene expression analysis*</button>
...
<button onclick="myFunction()">*Click here for details on microglia TF gene expression*</button>
