...
data. The output is a matrix of TF activity per cell. 
...
Transcription factor (TF) activity of 125 different transcription factors.
...
top_tfs %>% 
  ...
  geom_text(nudge_x = .05, nudge_y = 1.5) +
...
### Heatmap for just the HIV1 patient
...
non-parametric -> seems more appropriate in this case
* in both cases the Null-Hypothesis is that there is no difference
between the groups
* Posthoc analysis -> pairwise comparisons
  + pairwise wilcox (does not assume normal distribution, while t test does)
  -> adapt the p value, because repeatedly testing will increase the alpha
  error ->  p.adjust() -> use bonferroni (most conservative)
  + use Dunn's test
* Effect size of Kruskal-Wallis-Test -> eta-squared 
...
* numeric vector of p-values as input
* specify method

  + bonferroni -> p-values are multiplied by the number of comparisons
  + Holm, Hochberg, Hommel, Benjamini & Hochberg (less conservative)

I will use FDR, because it is more powerful. 
...
### For 24 TFs also identified by ATAC-seq

#### Kruskal-Wallis test and p-value adjustment 
...
* 16 of the 24 TFs from ATAC-seq data are differentially active between the three 
clusters.
...
* adjust p_values with fdr method
* since we have < 50 observations in some clusters the test returns a normal 
approximated p value

**16 TFs have significantly different activity between the 3 clusters.** 
**For these I performed a Wilcox test to see which of them are differen**
...
For the 24 TFs from ATAC-seq there are 14 differentially expressed between clusters
...
p1 <- ggplot(test, aes(x = ht_cluster, y = CTCF)) +
  geom_boxplot() +
...
p2 <- ggplot(test, aes(x = ht_cluster, y = NFIL3)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(NFIL3 ~ ht_cluster) %>% 
                       add_xy_position())

p3 <- ggplot(test, aes(x = ht_cluster, y = MXI1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(MXI1 ~ ht_cluster) %>% 
                       add_xy_position())

p4 <- ggplot(test, aes(x = ht_cluster, y = CREB1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(CREB1 ~ ht_cluster) %>% 
                       add_xy_position())

p5 <- ggplot(test, aes(x = ht_cluster, y = NFATC2)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(NFATC2 ~ ht_cluster) %>% 
                       add_xy_position())

p6 <- ggplot(test, aes(x = ht_cluster, y = MEF2A)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(MEF2A ~ ht_cluster) %>% 
                       add_xy_position())

p7 <- ggplot(test, aes(x = ht_cluster, y = LEF1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(LEF1 ~ ht_cluster) %>% 
                       add_xy_position())

p8 <- ggplot(test, aes(x = ht_cluster, y = YY1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(YY1 ~ ht_cluster) %>% 
                       add_xy_position())


p9 <- ggplot(test, aes(x = ht_cluster, y = MYB)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(MYB ~ ht_cluster) %>% 
                       add_xy_position())

p10 <- ggplot(test, aes(x = ht_cluster, y = ETS1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(ETS1 ~ ht_cluster) %>% 
                       add_xy_position())


p11 <- ggplot(test, aes(x = ht_cluster, y = JUNB)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(JUNB ~ ht_cluster) %>% 
                       add_xy_position())


p12 <- ggplot(test, aes(x = ht_cluster, y = CEBPB)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(CEBPB ~ ht_cluster) %>% 
                       add_xy_position())


p13 <- ggplot(test, aes(x = ht_cluster, y = ETV6)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(ETV6 ~ ht_cluster) %>% 
                       add_xy_position())



p14 <- ggplot(test, aes(x = ht_cluster, y = MEF2C)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(MEF2C ~ ht_cluster) %>% 
                       add_xy_position())

p15 <- ggplot(test, aes(x = ht_cluster, y = KLF4)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(KLF4 ~ ht_cluster) %>% 
                       add_xy_position())

figure <- ggarrange(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, 
                    p13, p14, p15, ncol = 5, nrow = 3)
annotate_figure(figure, top = text_grob("HIV1_CSF      k = 3 for k-means clustering (TFs from ATAC-seq)", 
...
figure <- ggarrange(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14,
                    ncol = 5, nrow = 3)
annotate_figure(figure, top = text_grob("HIV1_CSF      k = 4 for k-means clustering (TFs from ATAC-seq)", 
               color = "black", face = "bold", size = 25))
```
...
p7 <- ggplot(test, aes(x = ht_cluster, y = YY1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(YY1 ~ ht_cluster) %>% 
                       add_xy_position())
...
p8 <- ggplot(test, aes(x = ht_cluster, y = ETS1)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(ETS1 ~ ht_cluster) %>% 
                       add_xy_position())
...
p9 <- ggplot(test, aes(x = ht_cluster, y = CEBPB)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(CEBPB ~ ht_cluster) %>% 
                       add_xy_position())
...
p10 <- ggplot(test, aes(x = ht_cluster, y = ETV6)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(ETV6 ~ ht_cluster) %>% 
                       add_xy_position())
...
p11 <- ggplot(test, aes(x = ht_cluster, y = MEF2C)) +
  geom_boxplot() +
  stat_pvalue_manual(test %>% 
                       pairwise_wilcox_test(MEF2C ~ ht_cluster) %>% 
                       add_xy_position())
...
figure <- ggarrange(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, 
                    ncol = 3, nrow = 4)
annotate_figure(figure, top = text_grob("HIV1_CSF      k = 5 for k-means clustering (TFs from ATAC-seq)", 
               color = "black", face = "bold", size = 25))
```
...
reference <- RunPCA(reference, 
                    # by default a partial PCA is run, but we compute most singular values -> run standard SVD instead
                    approx = FALSE, 
                    verbose = FALSE)
...
### Kruskal-Wallis Test & Wilcox test

We will test which TFs are significantly different between clusters and then
use a Wilcox test to determine which clusters are different.
...
```
## K-means clustering with k = 3 for patient 1
...
```{r}
ggVennDiagram(list(hiv1_diff, diff_hiv2, diff_hiv3),
              label_alpha = 0,
...
```{r}
ggVennDiagram(list(hiv1_atac, hiv2_atac, hiv3_atac),
              label_alpha = 0,
              category.names= c("HIV1", "HIV2", "HIV3"),
              #set.size = 20,
              label = "count",
...
p1 <- ggplot(df, aes(x = ht_cluster, y = NFIL3)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(NFIL3 ~ ht_cluster) %>% 
                       add_xy_position())

p2 <- ggplot(df, aes(x = ht_cluster, y = MXI1)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(MXI1 ~ ht_cluster) %>% 
                       add_xy_position())
...
p3 <- ggplot(df, aes(x = ht_cluster, y = MEF2A)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(MEF2A ~ ht_cluster) %>% 
                       add_xy_position())
...
p4 <- ggplot(df, aes(x = ht_cluster, y = YY1)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(YY1 ~ ht_cluster) %>% 
                       add_xy_position())
...
p5 <- ggplot(df, aes(x = ht_cluster, y = ETS1)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(ETS1 ~ ht_cluster) %>% 
                       add_xy_position())
...
p6 <- ggplot(df, aes(x = ht_cluster, y = CEBPB)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(CEBPB ~ ht_cluster) %>% 
                       add_xy_position())
...
p7 <- ggplot(df, aes(x = ht_cluster, y = ETV6)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(ETV6 ~ ht_cluster) %>% 
                       add_xy_position())
...
p8 <- ggplot(df, aes(x = ht_cluster, y = KLF4)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(KLF4 ~ ht_cluster) %>% 
                       add_xy_position())
...
p9 <- ggplot(df, aes(x = ht_cluster, y = MEF2C)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(MEF2C ~ ht_cluster) %>% 
                       add_xy_position())
...
p10 <- ggplot(df, aes(x = ht_cluster, y = JUNB)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(JUNB ~ ht_cluster) %>% 
                       add_xy_position())

p11 <- ggplot(df, aes(x = ht_cluster, y = REST)) +
  geom_boxplot(aes(fill = ht_cluster)) +
  stat_pvalue_manual(df %>% 
                       pairwise_wilcox_test(REST ~ ht_cluster) %>% 
                       add_xy_position())

figure <- ggarrange(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, ncol = 4, nrow = 3)
...
auc_df <- auc_df %>% rownames_to_column("cell") %>% 
  mutate(ht_cluster = ifelse(cell %in% hiv1_c1, "hiv1_c1", 
         ifelse(cell %in% hiv1_c2, "hiv1_c2", 
         ifelse(cell %in% hiv1_c3, "hiv1_c3",
         ifelse(cell %in% hiv2_c1, "hiv2_c1",
         ifelse(cell %in% hiv2_c2, "hiv2_c2",
         ifelse(cell %in% hiv2_c3, "hiv2_c3",
         ifelse(cell %in% hiv3_c1, "hiv3_c1",
         ifelse(cell %in% hiv3_c2, "hiv3_c2", "hiv3_c3")))))))))
```
...
### Correlation between clusters based on 24 TFs with differential binding scores in different infection states in primary microglia
...
# Correlation between clusters for 11 differently active TFs
...
ht_cluster <- (
  hivmy2@meta.data %>% 
  rownames_to_column("cell") %>% 
  mutate(ht_cluster = ifelse(cell %in% c(hiv1_c1, hiv2_c1, hiv3_c3), "c1", "c2")) %>% 
  column_to_rownames("cell"))["ht_cluster"]
